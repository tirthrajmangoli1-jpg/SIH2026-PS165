...
export default function TrainingHubModal({ onClose, onTrainingCompleted }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        
        {/* Modal Header */}
        <div className="p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2.5 rounded-xl bg-amber-50 text-amber-700 border border-amber-200 shadow-sm">
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-slate-200 bg-white px-6 space-x-4">
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50/50">
          ...
          {successMsg && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 text-xs text-emerald-800 flex items-center space-x-2 shadow-sm">
          ...
          {activeTab === 'parameters' && (
            <div className="space-y-4">
              <div className="bg-white border border-slate-200 rounded-xl p-4 text-xs text-slate-600 shadow-sm">
                <span className="font-bold text-slate-900">Process-Safety Physics Thresholds:</span> The SIF-Sentinel neuro-symbolic engine applies hard deterministic energy bounds and barrier failure criteria defined by Indian & International oilfield regulations:
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {parameters.map((param) => (
                  <div 
                    key={param.id}
                    className="bg-white border border-slate-200 hover:border-slate-300 rounded-xl p-4 shadow-sm space-y-3 transition"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <h4 className="text-xs font-bold text-slate-900 mt-1.5">{param.parameter_name}</h4>
                    ...
                    <div className="bg-slate-50 border border-slate-200/80 rounded-lg p-2.5 text-[11px] space-y-1.5">
                      ...
                      <div>
                        ...
                        <span className="text-slate-700 font-medium">{param.iogp_mapping}</span>
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="text-sm font-bold text-slate-900">Fine-Tune & Calibrate AI Weights</h4>
                    <p className="text-xs text-slate-500 mt-0.5">
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="flex items-center space-x-2 text-xs">
                      <span className="text-slate-600 font-medium">Epochs:</span>
                      <select 
                        value={epochs} 
                        onChange={(e) => setEpochs(Number(e.target.value))}
                        className="bg-slate-50 border border-slate-300 rounded-lg px-2.5 py-1 text-slate-800 font-semibold focus:outline-none focus:ring-1 focus:ring-amber-500"
                      >
              ...
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                  <h4 className="text-xs font-bold text-slate-900">Epoch Convergence Log</h4>
                  <span className="text-[11px] text-slate-500 font-mono">Status: {statusData?.status || 'Trained'}</span>
                </div>

                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-100/60 text-slate-600 uppercase text-[10px] font-bold">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(statusData?.history || []).map((h, i) => (
                        <tr key={i} className="hover:bg-slate-50 font-mono">
                          <td className="py-2.5 px-4 font-bold text-slate-900">Epoch {h.epoch}</td>
                          <td className="py-2.5 px-4 text-slate-600">{h.loss.toFixed(4)}</td>
                          <td className="py-2.5 px-4 text-slate-600">{h.val_loss.toFixed(4)}</td>
          ...
          {activeTab === 'datasets' && (
            <div className="space-y-4">
              <div className="bg-white border border-slate-200 rounded-xl p-4 text-xs text-slate-600 shadow-sm">
                <span className="font-bold text-slate-900">Multi-Source Process-Safety Corpus:</span> SIF-Sentinel ingests real-world high-potential incident logs from 4 distinct enterprise & regulatory domains:
              </div>
              ...
              <div className="space-y-3">
                {datasetSources.map((ds) => (
                  <div 
                    key={ds.key}
                    className="bg-white border border-slate-200 hover:border-slate-300 rounded-xl p-5 shadow-sm space-y-3 transition"
                  >
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          Source: <span className="font-semibold text-slate-700">{ds.official_source}</span>
                        </p>
                      </div>
                      ...
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-amber-50 text-slate-700 hover:text-amber-800 border border-slate-200 text-xs font-semibold flex items-center space-x-1.5 transition"
                      >
