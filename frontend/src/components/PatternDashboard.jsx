...
export default function PatternDashboard({ patterns, onSelectTheme, onSelectIncidentId }) {
  ...
  const getCellBg = (yIdx, xIdx, count) => {
    ...
    if (count > 0) {
      ...
      if (risk >= 10) return 'bg-amber-500 text-slate-950 font-black ring-1 ring-amber-300 shadow-none';
      if (risk >= 6) return 'bg-yellow-400 text-slate-950 font-bold';
      return 'bg-emerald-500 text-white font-bold';
    }
    // Empty cells
    if (risk >= 16) return 'bg-rose-950/30/80 border-rose-200 text-slate-400';
    if (risk >= 10) return 'bg-amber-950/30/80 border-amber-800/50 text-slate-400';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-none">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2.5">
              <div className="w-8 h-8 rounded-xl bg-amber-500 text-slate-950 flex items-center justify-center font-bold shadow-none">
            ...
            <p className="text-xs text-slate-400 mt-2 max-w-4xl leading-relaxed font-medium">
      ...
      {/* Cluster Theme Cards */}
      <div>
        <h3 className="text-xs font-black text-slate-300 uppercase tracking-wider mb-3.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {clusters.map((cluster, idx) => (
            <button 
              key={idx}
              onClick={() => onSelectIncidentId && cluster.sample_incident_ids && cluster.sample_incident_ids.length > 0 && onSelectIncidentId(cluster.sample_incident_ids[0])}
              className="bg-slate-950 border border-slate-800 hover:border-amber-400 hover:shadow-md rounded-2xl p-5 transition shadow-none flex flex-col justify-between text-left cursor-pointer w-full"
            >
              <div>
                <div className="flex items-start justify-between">
                  <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-md bg-slate-800 text-slate-300 border border-slate-800">
                    Cluster #{cluster.cluster_id >= 0 ? cluster.cluster_id : 'Misc'}
                  </span>
                  {cluster.high_sif_count > 0 && (
                    <span className="text-[10px] font-black px-2.5 py-0.5 rounded-md bg-rose-900/40 text-rose-400 border border-rose-800/50">
                      {cluster.high_sif_count} High SIF
                    </span>
                  )}
                </div>
                <h4 className="font-black text-sm text-slate-100 mt-2.5 leading-snug">
                  {cluster.theme_name}
                </h4>
                <p className="text-xs text-amber-400 font-bold mt-1.5 flex items-center space-x-1">
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  <span>Dominant Rule: {cluster.dominant_iogp_rule}</span>
                </p>
              </div>

              <div className="mt-4 pt-3.5 border-t border-slate-800 text-xs text-slate-400 space-y-2">
                <div className="flex justify-between">
                  <span className="font-medium">Incident Count:</span>
                  <span className="font-black text-slate-100">{cluster.incident_count} reports</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-medium">Facilities:</span>
                  <span className="text-slate-200 text-[11px] font-semibold truncate max-w-[180px]" title={cluster.facilities_impacted.join(', ')}>
      ...
      {risk_matrix_5x5.cells && risk_matrix_5x5.cells.length > 0 && (
        <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-none space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center space-x-2.5">
              <div className="w-8 h-8 rounded-xl bg-sky-900/40 text-sky-700 flex items-center justify-center font-bold">
            ...
            {selectedCell && (
              <button
                ...
                className="text-xs text-amber-400 font-bold hover:underline font-mono"
              >
                Clear Matrix Filter
              </button>
            )}
            ...
            <table className="w-full text-center border-collapse">
              <thead>
                <tr>
                  ...
                  {risk_matrix_5x5.barrier_levels.map((b, idx) => (
                    <th key={idx} className="p-3 text-[11px] font-black text-slate-200 uppercase tracking-tight bg-slate-800 border-b border-slate-800">
                      {b}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {risk_matrix_5x5.energy_levels.map((e, yIdx) => (
                  <tr key={yIdx}>
                    <td className="p-3 text-[11px] font-mono text-left text-slate-200 bg-slate-900 border-r border-slate-800 font-bold truncate max-w-[220px]" title={e}>
                      {e}
                    </td>
          ...
          {selectedCell && selectedCell.incident_count > 0 && (
            <div className="p-4 rounded-xl bg-amber-950/30 border border-amber-800/50 text-xs text-slate-200 flex flex-wrap items-center justify-between gap-2 shadow-none">
              <div>
                <span className="text-amber-300 font-black">Selected Intersection: </span>
                <span className="font-bold">{selectedCell.energy_level} × {selectedCell.barrier_status} ({selectedCell.incident_count} reports)</span>
              </div>
              <div className="font-mono text-slate-300 font-bold flex flex-wrap gap-2 items-center">
                <span>View Reports:</span>
                {selectedCell.incident_ids.map(id => (
                  <button 
                    key={id}
                    onClick={() => onSelectIncidentId && onSelectIncidentId(id)}
                    className="px-2 py-0.5 rounded bg-amber-200 text-amber-300 hover:bg-amber-300 transition underline cursor-pointer"
                  >
                    {id}
                  </button>
                ))}
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-none">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
          <div>
            <div className="flex items-center space-x-2.5">
              <div className="w-8 h-8 rounded-xl bg-amber-900/40 text-amber-400 flex items-center justify-center font-bold">
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-4xl font-medium">
          ...
          {/* Switcher Tabs */}
          <div className="flex items-center bg-slate-800 p-1 rounded-xl border border-slate-800 self-start md:self-auto shrink-0">
            <button
              ...
              className={`flex items-center space-x-1.5 px-3.5 py-1.5 rounded-lg text-xs font-black transition ${
                rankingTab === 'sites'
                  ? 'bg-amber-500 text-slate-950 shadow-none'
                ...
                return (
                  <button 
                    ...
                    className={`text-left w-full cursor-pointer rounded-2xl p-5 border transition ${
                      isHighDensity 
                        ? 'bg-rose-950/30/60 border-rose-800/50 hover:border-rose-400 hover:shadow-md' 
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        <span className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-mono font-black ${
                          idx === 0 ? 'bg-red-600 text-white shadow-none' :
                          idx === 1 ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-300'
                        }`}>
                          #{idx + 1}
                        </span>
                        <div>
                          <h4 className="font-black text-sm text-slate-100">{site.facility}</h4>
                          <span className="text-xs text-slate-500 font-medium">{site.location} · {site.total_reports} logs</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className={`text-base font-black font-mono ${isHighDensity ? 'text-red-400' : 'text-amber-400'}`}>
                    ...
                    <div className="w-full bg-slate-800 rounded-full h-2.5 mt-3.5 overflow-hidden">
                      <div 
                    ...
                    <div className="grid grid-cols-2 gap-2.5 mt-3.5 text-xs">
                      <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 shadow-none">
                        <span className="text-slate-500 block text-[10px] uppercase font-bold">Mapped IOGP Rule</span>
                        <span className="font-black text-amber-400 flex items-center space-x-1 truncate mt-0.5">
                      ...
                      <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 shadow-none">
                        ...
                        <span className="font-black text-red-400 truncate block mt-0.5">
                    ...
                    <div className="mt-3.5 p-3 rounded-xl bg-slate-950 border border-amber-800/50 text-xs flex items-start space-x-2.5 shadow-none">
                      <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                      <div>
                        <span className="text-[10px] uppercase font-black text-amber-400 tracking-wider block">Recommended HSE Intervention:</span>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-black text-sm text-slate-100">{act.activity}</h4>
                    ...
                    <div className="mt-3.5 p-3 rounded-xl bg-slate-950 border border-amber-800/50 text-xs flex items-start space-x-2.5 shadow-none">
                      ...
                      <div>
                        <span className="text-[10px] uppercase font-black text-amber-400 tracking-wider block">Targeted HSE Focus:</span>
                        <p className="text-slate-300 text-xs font-medium leading-relaxed mt-0.5">{act.hse_focus_intervention}</p>
                      </div>
                    </div>
                  </button>
                );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-none">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h4 className="text-xs font-black text-slate-100 uppercase tracking-wide flex items-center space-x-1.5">
